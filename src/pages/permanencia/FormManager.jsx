...
import InfoPermanencia from 'components/Molecules/Info/InfoPermanencia'
...
function FormManager({ handleChangeStep }) {
    ...
    PermanenciaService.estudiante(
      ...
      (received) => {
        ...
        if (result.status) {
          ...
          if (result.message === 'Su permanecia ya esta registrada')
            changeStepIndex(5)
  ...
  const handleSubmit = () => {
    const sendData = {
      ...
      gestion_permanencia: +gestionCurrent + 1,
    }

    PermanenciaService.create(
      sendData,
      gestionCurrent,
      (received) => {
        const result = received.data
